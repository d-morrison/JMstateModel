...
JMstateModel <-
    ...
    ...)
  {
    ...
    #### Errors message ####
    {
      ...
      if (Mstate && (is.null(ID.Mstate) || !is.element(ID.Mstate, names(data.Mstate)) || is.null(data.Mstate))) {
        stop("\nwhen Mstate is TRUE, you must specify data.Mstate and ID.Mstate",
          " which respectively correspond to the database used in coxph() and the column name of the id in data.Mstate.")
      }
      if (CompRisk && Mstate) {
